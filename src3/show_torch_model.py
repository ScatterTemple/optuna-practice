...
def acqf_factory(ACQF_):
    ...
    class ModifiedACQF(ACQF_):
        ...
        def pof(self, X: torch.Tensor):
            ...
            cdf = 1. - normal.cdf(torch.tensor(0.5, device='cpu').double())
...
if __name__ == '__main__':
    ...
    def get_next_point():
        ...
        if show_fig:
            ...
            zz = 1 - torch.distributions.Normal(mean, std).cdf(torch.tensor(0.5)).detach().numpy().flatten().reshape(xx.shape)
            show_contour(zz, layout)
